...
GLuint platspec_loadpng(const char * filename){
    ...
    for (unsigned int y = 0; y < height; y++) row_pointers[y] = (png_bytep )malloc(rowbytes);
    png_read_image(png_ptr, (png_bytep*)row_pointers);
    fclose(fp);
    for (unsigned int y = 0; y < height; y++) memcpy(buffer+(rowbytes * y), row_pointers[y], rowbytes);
    ...
    for (unsigned int y = 0; y < height; y++)free(row_pointers[y]);
    free(row_pointers);
...
void platspec_init(){
    ...
    if (glewErr != GLEW_OK && glewErr != GLEW_ERROR_NO_GLX_DISPLAY)exit(-3);
